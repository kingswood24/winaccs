...
Uses
  ...
  VAR
    ...
    perc_over	  : SmallInt;	   { store the converted string overhead_perc }
    ...
    TXTOTAL    : INTEGER;
    STATUS     : SHORTSTRING{[1]}; //was string
    EDITST     : SHORTSTRING{[1]}; //was string
    TXQUAN     : Double{LONG};
    RUNBAL     : Double{LONG};
    STUBTYPE   : SHORTSTRING{[3]}; //was string
    print_inc_tot,
    ...
    TransType  : Integer;
    VatAmt     : Double{Long};
    VatCode    : SHORTString{[1]}; //was string
    EditStat   : SHORTString{[1]}; //was string
    NegSign    : Integer;
    ...
    *)
    ...
    PROCEDURE PRINTDETAIL (BANKTYPE: ShortString);
    VAR
      TxCounter,
      SUB      : INTEGER;
      SUB1     : INTEGER;
      ...
      OK       : BOOLEAN;
      ST       : ShortSTRING;
      TEST     : ShortString{STRING[1]};
      t_date,
      l_date,
      TESTSTUB : ShortString;//STRING{[8]};
      TESTTXT  : INTEGER;
      TESTDATE : INTEGER;
      STUB     : LONG;
      STUB1    : SmallInt;
      STUB2    : SmallInt;
      TXSTART  : INTEGER;
      TXDATE   : ShortString;//STRING{[8]};
      TXCOMM   : ShortString;//STRING{[15]};
      ANY      : BOOLEAN;
      BANK_REC : ShortString;//STRING{[1]};   (* temp var to store the bank_rec 'P','U' or 'E'*)
      curr_trans : integer;
      ...
      Procedure next_trans ( var update_read_no : SmallInt );
      Begin
...
// tgm comments 16/10/01
// calc_qty goes and gets the total amounts for each nominal for current year between the 2 dates entered
// returned to variables local_qty and local_amt
		If showbreakdown Then
		   Begin
		     If NOT calc_qty ( true, rec_count, Fromdt, ToDt, nom_name,
					local_qty, local_amt, NlFile, TxFile,
                                        AnalysisCodeFrom,AnalysisCodeTo ) then
			 If ( local_qty <= 0 ) then
			    local_qty := 0;
		      nomyr := local_amt;
		   End;
// tgm added next bit to get the share of evrheads from previous year 16/10/01
// holdtotal and HoldQty used to hold the totals fromt he current year so they can be added to the toals for the previous year
// I don't think the quatity is of any relevance for overheads, but I add it up anyway
// in case it may be needed sometime
                   If UsePreviousYear Then
                      Begin
                           ...
                           If NOT calc_qty ( true, rec_count, fromdt, todt, nom_name,
                                           local_qty, local_amt, LastNlFile, LastTxFile,
                                           AnalysisCodeFrom, AnalysisCodeTo ) Then
                                           If ( local_qty <= 0 ) Then
                                              local_qty := 0;

                           HoldTotal := HoldTotal + local_amt;
                           HoldQty   := HoldQty + local_qty;
                           local_Qty := HoldQty;
                           nomyr := HoldTotal;
                      End;
// don't think next 2 lines are needed - TGM 16/10/01
//                      Else
//                          nomyr := test_tot;
		...
		{ if the percentage rate is 0 then output all zeros }
		if percentage = 0 then
		   nomyr := 0;
//tgm check this overhead percent calc
                // Calculate the percentage
                if (( nomyr < 0 ) and ( percentage > 0 )) then
		   nomyr := ( nomyr * ( percentage / 100 ))
		else
		   if (( nomyr > 0 ) and ( percentage > 0 )) then
		      nomyr := ( nomyr * ( percentage / 100 ));
	...
	BEGIN						{ begin entvalue }
          ...
          IF ITEM=13
	    THEN BUILDLINE (125,	WORKSTR) // was 125
	    ELSE BUILDLINE (21+(ITEM*8),WORKSTR); //(34+(ITEM*7),WORKSTR);
...
(* change *)		  6 : BEGIN
				...
				if act = 6 then
				  begin
				    If ShowBreakdown Or UsePreviousYear Or
                                       ( FromDt <> CDateFrom ) Or
                                       ( ToDt <> CDateTo ) Then
				      Begin
					this_year_per_qty := 0;
...
{-->>                                        HoldQty   := 0;}
					...
					If NOT calc_qty ( true, sub, fromdt, todt, nom_name,
						 nominal_qty, test_tot, NlFile, TxFile,
                                                 AnalysisCodeFrom, AnalysisCodeTo ) Then
					  If ( nominal_qty <= 0 ) Then
					    nominal_qty := 0;

// the next bit needs repeated somewhere else to sort out the overheads from previous year. TGM 16/10/01
// calc_qty does the job of getting the transacvtion between the dates, both quantity and amount
                                        If UsePreviousYear Then
                                           Begin
                                              HoldTotal := Test_Tot;
                                              HoldQty   := Nominal_Qty;
                                              ...
                                              If NOT calc_qty ( true, sub, fromdt, todt, nom_name,
					   	              nominal_qty, test_tot, LastNlFile, LastTxFile,
                                                              AnalysisCodeFrom, AnalysisCodeTo ) Then
                                                 If ( nominal_qty <= 0 ) Then
					            nominal_qty := 0;
	...
	IF INCF > 0 THEN ENTNOMINAL (INCF, INCT, INCTOT, NETTOT, TRUE, FALSE);
      ...
      PROCEDURE ENTTOTAL (TITLE: ShortString{STRING}; VAR TOTAL: LISTCOLS; SUM: BOOLEAN);
      Var
         X : Integer;
	  ...
	  4,5 : BEGIN
		  ...
		  FOR X:=1 TO 12 DO
		    BEGIN
		      ...
		      BUILDLINE (21+(X*8),WORKSTR); // was 34 + (X*7)
		    END;
		  DoubleToStr (TOTAL[13],WORKSTR,'%8.0f', True, True, 8, True);
		  BUILDLINE (125,WORKSTR); // was 125
		END;
	  3   : FOR X:=1 TO 7 DO
		  BEGIN
...
(* change *)
			...
			Begin
			  ...
			  if showbreakdown then
				...
				End;
			      ...
			      DoubleToStr ( exp_this_year, workstr, '%f' , True,  True,11, True);
			      buildline ( 97, workstr );
			    End;
			End;
	...
	IF ESCKEY THEN EXIT;
      ...
      PROCEDURE ENTDIV (TITLE: ShortString{STRING}; TOTAL: LISTCOLS; UNITS: Double );
      VAR
	...
	BEGIN
	  ENTDASH;
	  ENTTOTAL (CONCAT('TOTAL ',Cash2.XENTUPPER,' INCOME'),INCTOT,TRUE);    // GM BUILD THE INCOME LINES FOR ENT REPORT
	  ENTDASH;
	END;
      IF PROG=SPECREP THEN
	BEGIN
	...
	BEGIN
	  IF EXPF>0 THEN
		...
		*)
		...
		ENTDASH;
	      END;
	  ENTTOTAL (CONCAT('TOTAL ',Cash2.XENTUPPER,' EXPENSES'),EXPTOT,TRUE);      // total ent expenses line
	  ENTDASH;
	  BLANKNEXT;
	  OUTPUTLINE;
	  IF ESCKEY THEN EXIT;
	  ...
	  if (( act = 6 ) and ( perc_over > 0 ) and ( perc_over <= 100 )) then
	    Begin
	      ...
	      entdash;
              BlankNext;  //TGM AB - 02/11/12 blank the line first
		...
		expend_tot := ((( exptot[1] * 10000 )
	      ...
	      DoubleToStr ( total_per_units+ ( exptot[1] / (no_units / 100 )), workstr, '%f' , True,  True,11, False);
	      buildline ( 61, workstr );
	  ...
	  ELSE
	    BEGIN
              ...
              // TGM AB 2/11/12
              FOR SUB:=1 TO 13 DO
		IF (NETTOT[SUB]<>0) AND (INCTOT[SUB]<>0) THEN PROFIT[SUB]:=NETTOT[SUB];
                ENTTOTAL (CONCAT(Cash2.XENTUPPER,' GROSS MARGIN  '),PROFIT,FALSE);
	  ...
	  if not SINGLE_ENT then
	    ...
	    begin
          ...
          if EntCostPerUnit.PencePerUnit.checked then begin
             DoubleToStr ( (TempUnits*100) , WorkStr, '%f' , True,  True,11, False);
          end;

	  BUILDLINE ( 1,CONCAT('NUMBER OF ',Cash2.XENTUPPER,' UNITS'));
	  CASE ACT OF
	    4,5 : BUILDLINE (33{37}{39},WORKSTR);             //Ch030 was 37
	    3	: BUILDLINE (30,WORKSTR);        // TGM AB Editing was 43
	    1,2,
	  ...
	  IF ESCKEY THEN EXIT;
          ...
          ENTDIV (CONCAT('INCOME/',UNITDESC),INCTOT,TempUnits{CURRLONG});
	  ENTDIV (CONCAT('EXPENDITURE/',UNITDESC),EXPTOT,TempUnits{CURRLONG});
  ...
  var
    ...
    BEGIN					   { begin buildname }
      ...
      *)
      ...
      if store_type in [1..3,11..12] then  // TGM AB 8/08/02 - added 11..12 to include account name on VAT Audit for Cashbook
	begin
	  setdb ( SLFile );
	  if store_acc <> 0 then begin
	...
	if store_type in [5..7,15..16] then  // TGM AB 8/08/02 - added 15..16 to include account name on VAT Audit for Cashbook
	  begin
	    setdb ( PLFile );
            if store_acc <> 0 then begin
  ...
  PROCEDURE buildcustsuppcode ( DB, ITEM, COL : INTEGER);                                         // Start Ch028
  var                                                                                             // biasc copy of buildname to append Cxxxx or Sxxxx to a linwe to correspond to customer supplier ID
    store_type,
    store_acc,
    store_db  : integer;
    name      : ShortString;{[30]}
  ...
  BEGIN			   { this is the being of PRINTOUT of CBA.NSREPB.TEXT }
    ...
    IF ( Device[1] in ['C','F','P','S'] ) AND ( NOT NUMBERONE ) AND (( DBNO = EntFile )
	...
	CASE ACT OF
	  30	: Begin				{ LIST TRANSACTIONS REPORT }
		    ...
		    builditem ( TxFile, 5, 54, 0 );	{ Description }   // TGM AB was 52
		    builditem ( TxFile, 26, 114, 0 ); { Stub-No }        // TGM AB Was 69                  // Ch028 was 110

                    buildcustsuppcode ( TxFile, 1, 124);                   // Ch028

                    // GM 18/02/03 then next 3 lines were an attempt to output nominal codes on audit trail -
                    // not a success as only the header trans of invoice is printed.
                    // this is the place however where the transaction listing report is built
                    // builditem puts the item into the output line
		    //builditem ( TxFile, 18, 80, 0 ); { Stub-No }
		    //builditem ( TxFile, 19, 90, 0 ); { Stub-No }
		    //buildline ( 104, tr_desc );	{ set by tx_type in processfile}
		    buildline ( 84, tr_desc );	{ set by tx_type in processfile}
                    // gm meddled with this 18/02/03 to get nominal unto transaction report - did not complete
		    //buildline ( 104, IntToStr(tr_debit) );	{ set by tx_type in processfile}
...
(* change *)
		    ...
		    BUILDITEM (NlFile,1,17,0);
	   	    BUILDITEM (NlFile,5,50,0);
                    if currint > 0 then begin ReadTables ( NlFile, Currint, CurrStr );     // TGM AB 29/09/17 - Currint holds the report group called up in Builditem line, this gets the name
                                              BUILDLINE (80,Currstr);    // TGM AB 29/09/17 - appends report group name onto end of report string
                    end;  // TGM AB 29/09/17
		  END;
	  7	: BEGIN           // tgm looks like this is the nominal audit trail rep code
                                  // TGM AB this secion generates the nominal code & name report heading
		    GETITEM (NlFile,15);
		    (*
	  ...
	  4	: BEGIN
		  ...
		  END;
	  6,21	: BEGIN
		    If act = 6 Then          // TGM| AB TRIAL BALANCE STUFF
               //     currrec[dbno] := Schrecf;  // TGM AB 04/02/05 - Fixes problem with currec not being set
		      Begin
                           ...
                           WorkStr[NegSign] := ' ';
{
    ...
    IF ESCKEY THEN EXIT;
    NUMBERONE:=FALSE;
    IF (ACT IN [7,4,5]) AND (PROG=SPECREP) THEN  // tgm this bit run if nominal audit trail
      BEGIN
	GETITEM(NlFile,22);
	RUNBAL:=CURRLONG;
	CASE ACT OF
